...
export default async function OpportunityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="shell">
    ...
    {item.company_number && <section className="panel source-intelligence"><h2>Corporate proprietor intelligence</h2><dl className="evidence-facts"><div><dt>Company</dt><dd>{item.proprietor_name ?? "Name pending enrichment"}</dd></div><div><dt>Company number</dt><dd>{item.company_number}</dd></div><div><dt>Companies House status</dt><dd>{item.company_status ?? "Not yet enriched"}</dd></div></dl><a className="button-link" href={`https://find-and-update.company-information.service.gov.uk/company/${encodeURIComponent(item.company_number)}`} target="_blank" rel="noreferrer">Open Companies House profile ↗</a></section>}
